...
from regulAItor.Framework.settings import InputGuardrail, GenerateSummary
...
class FDAWarningLetterSystem:
    ...
    def setup_qdrant(self) -> None:
        ...
        if self.client.get_collection(self.qdrant_config.collection_name):
            try:
        ...
        self.client.create_collection(
            ...
            payload_schema={
                "recommendations": PayloadSchema(data_type="text"),
                "warning_letter": PayloadSchema(data_type="text"),
                "summary": PayloadSchema(data_type="text")
            }
        )
    ...
    def setup_llm_config(self) -> None:
        ...
        vectorizer = dspy.OpenAIVectorizer(model="text-embedding-ada-002")

        # Initialize Qdrant retriever for DSPy
        qdrant_retriever = dspy.QdrantRM(
    ...
    def initialize_agents(self) -> None:
        ...
        self.review_agent = AssistantAgent(
             name="Review_Agent",
             system_message="""Evaluate the proposed corrective actions to ensure they are comprehensive, proper, and fully address all regulatory violations outlined in the original warning letter. 
    ...
    def retrieve_similar_cases(self, summary_embedding: List[float]) -> List[Dict[str, Any]]:
    ...
    def process_warning_letter(self, warning_letter: str) -> Dict[str, Any]:
        ...
        while not review_result.get('Approved', 'No') == 'Yes' and revision_count < max_revision_attempts:
            logger.info(f"Revision attempt {revision_count + 1}: Improving corrective actions...")
    ...
    def reset_agents(self) -> None:
        ...
        for agent in [self.summary_generator, self.retrieval_agent, self.regulation_agent,
                      self.law_provider, self.corrective_agent, self.review_agent]:
            agent.reset()