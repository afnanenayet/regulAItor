...
class FDALetterProcessor:
    def __init__(self, config: ProcessorConfig):
        ...
        self.summarizer_agent = ConversableAgent(
            name="Summarizer",
            system_message="""You are an FDA regulatory expert. Extract from the warning letter:
1. All violated terms/regulations with specific citations.
2. Concrete recommendations for compliance.
...
""",
    ...
    async def process_letter(self, letter_name: str, content: str) -> Optional[Dict]:
        ...
        try:
            ...
            summarization_response = self.extract_assistant_response(summarization_chat, letter_name)
            if summarization_response is None:
                return None

            # Parse the summarization response
            summarization_result = self.parse_json_response(summarization_response, letter_name, "summarization")
            ...
            for attempt in range(self.config.max_validation_attempts):
                ...
                validation_response = self.extract_assistant_response(validation_chat, letter_name)
                if validation_response is None:
                    return None

                # Parse the validation response
                validation_result = self.parse_json_response(validation_response, letter_name, "validation")
                ...
                if validation_result.get("status") == "APPROVED":
                    ...
                    logging.warning(f"Validation rejected for {letter_name} on attempt {attempt + 1}. Revising...")
                    ...
                    logging.error(f"Invalid status in validation result for {letter_name}: {validation_result.get('status')}")
                    return None
    ...
    def extract_assistant_response(self, chat_result, letter_name: str) -> Optional[str]:
        """
    ...
    def parse_json_response(self, response_content: str, letter_name: str, stage: str) -> Optional[Dict]:
        """
        ...
        try:
            ...
            if json_match:
                ...
                logging.error(f"No JSON object found in {stage} response for {letter_name}: {response_content}")
                return None
